...
export async function GET(
  ...
  try {
    const invitations = await getInvitationsForResource('ORGANIZATION', params.id)
    return apiSuccess(invitations)
  } catch (err: any) {
    return apiError(err.message || 'Failed to list invitations', 500)
  }
}
...
export async function POST(
  ...
  try {
    ...
    return apiError(err.message || 'Failed to create invitation', 500)
  }
}
